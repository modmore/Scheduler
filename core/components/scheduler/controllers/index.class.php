...
class SchedulerIndexManagerController extends SchedulerManagerController
{
    ...
    public function loadCustomCssJs()
    {
        ...
        $this->addCss($this->scheduler->config['cssUrl'] . 'bootstrap.buttons.css');
        $this->addJavascript(
            $this->scheduler->config['managerUrl'] . 'assets/modext/widgets/core/modx.grid.local.property.js'
        );
        $this->addJavascript($this->scheduler->config['managerUrl'] . 'assets/modext/util/datetime.js');

        $this->addJavascript($this->scheduler->config['jsUrl'] . 'mgr/misc/scheduler.utils.js');
